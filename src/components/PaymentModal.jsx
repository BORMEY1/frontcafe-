...
const PaymentModal = ({ isOpen, onClose, onPaymentSuccess, isGuest = false }) => {
  ...
  // Note: 'total' from useCart already includes 10% tax.
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      if (isGuest) {
        toast.success('Order confirmed! Please wait 5-8 minutes.', {
          duration: 5000,
          icon: '✅',
        });
      } else {
      ...
      <div className="glass-card w-full max-w-lg p-6 rounded-[2.5rem] relative overflow-hidden flex flex-col max-h-[90vh] border border-white/10 shadow-2xl">
        ...
        <div className="overflow-y-auto pr-2 custom-scrollbar">
          ...
          <div className="space-y-6 mb-8">
             <div>
                <label className="block text-gray-400 text-[10px] font-black uppercase tracking-[0.2em] mb-3 ml-1">1. How will you enjoy your meal?</label>
                <div className="grid grid-cols-3 gap-2">
                   {[
                     { id: 'Eat In', icon: <Utensils size={18} /> },
                     { id: 'Take Away', icon: <ShoppingBag size={18} /> },
                     { id: 'Delivery', icon: <Truck size={18} /> }
                   ].map(item => (
                     <button
                       key={item.id}
                       onClick={() => setOrderType(item.id)}
                       className={`py-4 px-2 rounded-2xl border font-bold transition-all flex flex-col items-center justify-center gap-2 ${
                         orderType === item.id 
                          ? 'bg-primary-600 border-primary-400 text-white shadow-lg' 
                          : 'bg-dark-900 border-white/5 text-gray-500 hover:border-white/10 active:scale-95'
                       }`}
                     >
                        {item.icon}
                        <span className="text-[9px] uppercase tracking-tighter">{item.id}</span>
                     </button>
                   ))}
                </div>
             </div>
                        ...
                        <button
                          ...
                          className={`py-3 rounded-xl border font-bold transition-all ${
                            tableNumber === num.toString()
                             ...
                             : 'bg-dark-900 border-white/5 text-gray-400 hover:border-white/10 active:scale-90'
                          }`}
                        >
                           {num}
                        </button>
             ...
             <div className="flex gap-2 mb-6 bg-dark-900/50 p-1 rounded-2xl">
               {['Cash', 'QR'].map((m) => (
                 <button
                   key={m}
                   onClick={() => setMethod(m)}
                   className={`flex-1 flex items-center justify-center gap-2 py-3.5 rounded-xl transition-all font-bold ${
                     method === m ? 'bg-primary-600 text-white shadow-lg' : 'text-gray-500 hover:text-gray-300 active:scale-95'
                   }`}
                 >
                   {m === 'Cash' ? <Banknote size={18} /> : <QrCode size={18} />}
                   {m}
                 </button>
               ))}
             </div>
          ...
          <div className="bg-white/5 rounded-2xl p-5 border border-white/5 space-y-2 mb-6">
             ...
             <div className="flex justify-between text-xs font-bold text-gray-500">
                <span className="uppercase tracking-widest">VAT (10%)</span>
                <span className="font-mono">${tax.toFixed(2)}</span>
             </div>
             <div className="flex justify-between pt-3 border-t border-white/10">
                <span className="text-primary-500 font-black uppercase tracking-tighter text-sm italic">Grand Total</span>
                <span className="text-3xl font-black text-primary-500 font-mono">${finalTotal.toFixed(2)}</span>
             </div>
          </div>
          ...
          <div className="min-h-[140px]">
            {method === 'Cash' && (
              <div className="space-y-6 animate-in fade-in duration-300">
                {isGuest ? (
                  <div className="bg-primary-600/10 border border-primary-500/20 rounded-3xl p-6 text-center shadow-inner">
                    <div className="w-14 h-14 bg-primary-500 text-white rounded-full flex items-center justify-center mx-auto mb-4"><Banknote size={28} /></div>
                  ...
                  <>
                    <input type="number" value={cashInput} onChange={(e) => setCashInput(e.target.value)} placeholder="0.00" className="w-full bg-dark-900 border border-white/10 rounded-2xl px-5 py-5 text-4xl font-black text-white focus:border-primary-500 transition-all outline-none" autoFocus />
        ...
        <div className={`mt-6 pt-4 flex ${isGuest ? 'justify-end' : ''}`}>
           <button onClick={handlePayment} className={`${isGuest ? 'w-auto px-12 py-4' : 'w-full py-5'} bg-primary-600 hover:bg-primary-500 text-white font-black rounded-full transition-all shadow-[0_0_30px_rgba(234,179,8,0.3)] active:scale-90 text-sm uppercase tracking-widest flex items-center justify-center gap-3 relative overflow-hidden group`}>
            <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/10 to-transparent -translate-x-full group-hover:animate-[scan_2s_linear_infinite]"></div>
            <CheckCircle size={20} strokeWidth={3} />
            <span>{isGuest ? 'Confirm' : 'Confirm Order'}</span>
          </button>
        </div>
      </div>
    </div>
